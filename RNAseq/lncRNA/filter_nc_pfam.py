...
@click.command()
@click.option('-c', '--coding_pfam', type=click.Path(exists=True), required=True,
              help='pfam prediction for coding sequences.')
@click.option('-n', '--noncoding_pfam', type=click.Path(exists=True), required=True,
              help='pfam prediction for noncoding sequences.')
@click.option('-p', '--pfam_hit', type=click.File('w'), required=True,
...
def main(coding_pfam, noncoding_pfam, pfam_hit):
    ...
    for each_hit in cd_count:
        if each_hit in nc_count:
            cd_hit, cd_nohit = cd_count[each_hit], cd_total - cd_count[each_hit]
            nc_hit, nc_nohit = nc_count[each_hit], nc_total - nc_count[each_hit]
            oddsratio, pvalue = stats.fisher_exact([[cd_hit, cd_nohit], [nc_hit, nc_nohit]])
