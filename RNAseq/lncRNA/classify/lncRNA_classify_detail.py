...
parser.add_argument(
    '--lnc_classify', help='FEELnc_classifier output result', required=True)
    ...
    if direction == 'antisense':
        if lnc_type == 'intergenic':
            if subtype == 'divergent':
                ...
                sys.exit('error combination: %s' % ','.join(
                    [direction, lnc_type, subtype, location]))
        elif lnc_type == 'genic':
            if location == 'exonic':
...
with open(args.lnc_classify) as lnc_classify_info:
    for n, eachline in enumerate(lnc_classify_info):
        ...
        if n == 0:
            ...
            if isBest == '1':
                lnc_classify_dict[lncRNA_transcript] = [
                    score, lnc_class, output_line]
#            else:
#                if lnc_type == 'genic':
