...
for each_sample in sample_list:
    sample_info_dict[each_sample] = [0, 0, 0, 0, []]
    for n in (1, 2):
        ...
        with open(each_qc_file) as each_qc_file_info:
            ...
            for eachline in each_qc_file_info:
                ...
                if eachline.startswith("%GC"):
                    ...
                    sample_info_dict[each_sample][4].append(gc_content)
                ...
                if not eachline.startswith('#') and q30_flag == 1:
                    if int(float(eachline_info[0])) >= 20:
                        sample_info_dict[each_sample][2] += int(
                            float(eachline_info[1]))
                    if int(float(eachline_info[0])) >= 30:
                        sample_info_dict[each_sample][3] += int(
                            float(eachline_info[1]))

print 'Sample_ID\tReads_number(M)\tReads_length(bp)\tData_size(G)\tQ20(%)\tQ30(%)\tGC(%)'
for each_sample in sample_list:
    ...
    q20 = round(sample_info_dict[each_sample][2] *
                100 / float(sample_info_dict[each_sample][1]), 2)
    q30 = round(sample_info_dict[each_sample][2] *
                100 / float(sample_info_dict[each_sample][1]), 3)
    gc_content = sum(sample_info_dict[each_sample][4]) / \
        float(len(sample_info_dict[each_sample][4]))
    print '%s\t%s\t%s\t%s\t%s\t%s' % (each_sample, reads_num, read_length, data_size, q20, q30, gc_content)
